...
func JLSHandler(c *Conn, tlsError error) error {
	if c.extraConfig == nil || !c.extraConfig.UseJLS {
		return tlsError
	}
...
func BuildJLSClientHello(c *Conn, hello *clientHelloMsg) {
	if c.extraConfig == nil || !c.extraConfig.UseJLS || c.IsBuildedFakeRandom {
		return
	}
	zeroArray := BuildZeroArray()
	hello.random = zeroArray
	withoutBinder, _ := hello.marshalWithoutBinders()
	hello.random, _ = BuildFakeRandom(c.extraConfig, withoutBinder)
...
func BuildJLSServerHello(c *Conn, hello *serverHelloMsg) {
	...
	hello.random, _ = BuildFakeRandom(c.extraConfig, hello.raw)
	copy(hello.raw[6:], hello.random)
}
...
func CheckJLSServerHello(c *Conn, serverHello *serverHelloMsg) {
	...
	c.IsValidJLS, _ = CheckFakeRandom(c.extraConfig, raw, serverHello.random)
...
func CheckJLSClientHello(c *Conn, clientHello *clientHelloMsg) (bool, error) {
	...
	c.IsValidJLS, err = CheckFakeRandom(c.extraConfig, raw, clientHello.random)
	if err != nil {
...
func BuildFakeRandom(config *ExtraConfig, AuthData []byte) ([]byte, error) {
...
func CheckFakeRandom(config *ExtraConfig, AuthData []byte, random []byte) (bool, error) {
	iv := append(config.JLSIV, AuthData...)
	pwd := append(config.JLSPWD, AuthData...)
	fakeRandom := r.NewFakeRandom(pwd, iv)
